...
export default function QuotationsPage() {
  ...
  const [pharmacyProfiles, setPharmacyProfiles] = useState<Record<string, PharmacyProfile>>({})
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const prescriptionsArr: Prescription[] = Array.isArray(prescriptions) ? (prescriptions as Prescription[]) : []
        ...
        const pharmacyIds = Array.from(new Set(allQuotes.map((q) => q.pharmacy_id)))
  ...
  const handleStatusChange = async (
    ...
    try {
      ...
      if (newStatus === "accepted") updateFields.accepted_at = new Date().toISOString()
      if (newStatus === "rejected") updateFields.rejected_at = new Date().toISOString()
      ...
      toast({
        title: `Quotation ${newStatus === "accepted" ? "accepted" : "rejected"}!`,
        description:
          newStatus === "accepted"
      ...
      setQuotes((prev) =>
        prev.map((q) =>
          q.id === quote.id
            ? { ...q, status: newStatus, accepted_at: newStatus === "accepted" ? new Date().toISOString() : q.accepted_at, rejected_at: newStatus === "rejected" ? new Date().toISOString() : q.rejected_at }
            : q
        )
      )
    } catch (err: any) {
      ...
      {loading ? (
        <Card>
          <CardContent className="p-8 md:p-12 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">Loading quotations...</h3>
          </CardContent>
        </Card>
      ) : error ? (
        <Card>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex-1">
                    <CardTitle className="flex flex-col md:flex-row md:items-center gap-2 mb-2">
                      <span className="text-lg md:text-xl">{quotation.pharmacyProfile?.name || "Pharmacy"}</span>
                    ...
                    <CardDescription className="text-sm">
                      {quotation.pharmacyProfile?.address || ""} • {quotation.pharmacyProfile?.phone || ""}
                    </CardDescription>
                  </div>
                    ...
                    <p className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white">
                      ${
                        Array.isArray(quotation.items)
                          ? quotation.items.reduce((sum: number, item: any) => sum + (parseFloat(item.price || item.amount) || 0), 0) + (quotation.delivery_fee || 0)
                          : 0
                      }.00
                    </p>
                    <p className="text-sm text-gray-500">+ ${quotation.delivery_fee?.toFixed(2) || "0.00"} delivery</p>
                  </div>
                </div>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-gray-900 dark:text-white">{quotation.estimated_delivery}</p>
                    </div>
                  </div>
                    ...
                    <Dialog>
                      ...
                      <DialogContent className="max-w-2xl max-h-[90vh] overflow-auto">
                        <DialogHeader>
                          ...
                          <DialogDescription>
                            Quotation #{quotation.id} from {quotation.pharmacyProfile?.name || "Pharmacy"}
                          </DialogDescription>
                        </DialogHeader>
                        <div className="space-y-6">
                          ...
                          <div>
                            <h3 className="font-medium text-gray-900 dark:text-white mb-2">Pharmacy Information</h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                              <div>
                                <p className="text-gray-600 dark:text-gray-400">Name</p>
                                <p className="font-medium">{quotation.pharmacyProfile?.name}</p>
                              </div>
                              <div>
                                <p className="text-gray-600 dark:text-gray-400">Phone</p>
                                <p className="font-medium">{quotation.pharmacyProfile?.phone}</p>
                              </div>
                              <div className="md:col-span-2">
                                <p className="text-gray-600 dark:text-gray-400">Address</p>
                                <p className="font-medium">{quotation.pharmacyProfile?.address}</p>
                              </div>
                            </div>
                          </div>
                          {/* Items */}
                          ...
                          <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                            <div className="flex justify-between items-center mb-2">
                              ...
                              <span className="font-medium">
                                ${
                                  Array.isArray(quotation.items)
                                    ? quotation.items.reduce((sum: number, item: any) => sum + (parseFloat(item.price || item.amount) || 0), 0)
                                    : 0
                                }.00
                              </span>
                            </div>
                            <div className="flex justify-between items-center mb-2">
                              ...
                              <span className="font-medium">${quotation.delivery_fee?.toFixed(2) || "0.00"}</span>
                            </div>
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                Are you sure you want to accept this quotation from {quotation.pharmacyProfile?.name} for $
                                {
                                  Array.isArray(quotation.items)
                                    ? quotation.items.reduce((sum: number, item: any) => sum + (parseFloat(item.price || item.amount) || 0), 0) + (quotation.delivery_fee || 0)
                                    : 0
                                }
                                ? The pharmacy will be notified and will prepare your order.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                Are you sure you want to reject this quotation from {quotation.pharmacyProfile?.name}? This
